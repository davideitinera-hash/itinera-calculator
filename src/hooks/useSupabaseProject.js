...
function dbToState(project, equipment, transport, intStaff, extStaff, costs, phases, subprojects, accommodations, planData, whData, acctData) {
    return {
        ...
        intStaff: intStaff.map(s => ({ id: s.id, role: s.role, count: s.count, costHour: Number(s.cost_hour), hOrd: Number(s.h_ord), hStr: Number(s.h_str), hFest: Number(s.h_fest), hNott: Number(s.h_nott), days: s.days ?? 1, sellTotal: Number(s.sell_total || 0), subprojectId: s.subproject_id || null })),
        extStaff: extStaff.map(s => ({ id: s.id, role: s.role, count: s.count, costHour: Number(s.cost_hour), hOrd: Number(s.h_ord), hStr: Number(s.h_str), hFest: Number(s.h_fest), hNott: Number(s.h_nott), days: s.days ?? 1, sellTotal: Number(s.sell_total || 0), subprojectId: s.subproject_id || null })),
...
export function useSupabaseProject(projectId) {
    ...
    const addItem = useCallback(async (listField, newItem) => {
        const tableMap = {
            ...
            intStaff: { table: 'staff_entries', transform: (item) => ({ project_id: projectId, staff_type: 'internal', role: item.role || '', count: item.count || 1, cost_hour: item.costHour || 20, h_ord: item.hOrd || 8, h_str: item.hStr || 0, h_fest: item.hFest || 0, h_nott: item.hNott || 0, days: item.days ?? 1, sell_total: item.sellTotal || 0, subproject_id: item.subprojectId || null }) },
            extStaff: { table: 'staff_entries', transform: (item) => ({ project_id: projectId, staff_type: 'external', role: item.role || '', count: item.count || 1, cost_hour: item.costHour || 15, h_ord: item.hOrd || 8, h_str: item.hStr || 0, h_fest: item.hFest || 0, h_nott: item.hNott || 0, days: item.days ?? 1, sell_total: item.sellTotal || 0, subproject_id: item.subprojectId || null }) },
    ...
    const updateItem = useCallback(async (listField, itemId, field, value) => {
        ...
        const fieldMaps = {
            ...
            intStaff: { role: 'role', count: 'count', costHour: 'cost_hour', hOrd: 'h_ord', hStr: 'h_str', hFest: 'h_fest', hNott: 'h_nott', days: 'days', sellTotal: 'sell_total' },
            extStaff: { role: 'role', count: 'count', costHour: 'cost_hour', hOrd: 'h_ord', hStr: 'h_str', hFest: 'h_fest', hNott: 'h_nott', days: 'days', sellTotal: 'sell_total' },
